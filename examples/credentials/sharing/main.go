...
func handleCredentialPresentationResponse(selfAccount *account.Account, msg *event.Message) {
	...
	credentials, err := graph.ValidCredentialsFor(user.Reference.DocumentAddress())
	if err != nil {
		log.Fatal("user credentials invalid", "error", err)
	}

	emailAddress, ok := credentials[0].CredentialSubjectClaim(credential.FieldSubjectEmailAddress)
	if !ok {
		log.Fatal("expected email address")
	}

	log.Println("user email address:", emailAddress)
}

func requestUserCredentials(id int) {
	mu.Lock()
	user := users[id]
	mu.Unlock()
	...
	content, err := message.NewCredentialPresentationRequest().
		PresentationType(credential.PresentationTypeContactDetails).
		Authenticate(user.Reference, challenge).
		Predicates(predicate.NewTree(predicates)).
