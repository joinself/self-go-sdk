...
func TestMessagingSend(t *testing.T) {
	...
	err = c.Send([]string{"alice:1", "bob:1", "charlie:1"}, "test.message", 1, []byte("test"))
	require.Nil(t, err)
...
func TestMessagingRequest(t *testing.T) {
	...
	_, response, err := c.Request([]string{"alice:1"}, "1", "test.message", 1, []byte(`{"payload":"eyJjaWQiOiAiMSJ9"}`), 0)
	require.Nil(t, err)
