...
// Challenge returns the 32 byte challenge generated by the requester. returns nil if not specified
func (c *CredentialPresentationRequest) Challenge() []byte {
	return C.GoBytes(
		unsafe.Pointer(C.self_message_content_credential_presentation_request_challenge(
			c.ptr,
		)),
		32,
	)
...
// Challenge specifies a 32 byte challenge that the responder must include in their
func (b *CredentialPresentationRequestBuilder) Challenge(challenge []byte) *CredentialPresentationRequestBuilder {
	challengeBuf := C.CBytes(challenge)
	defer C.free(challengeBuf)

	C.self_message_content_credential_presentation_request_builder_challenge(
		b.ptr,
		(*C.uint8_t)(challengeBuf),
	)
